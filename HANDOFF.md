...
audit — the codebase was already disciplined, so these close the residual gaps). All changes are **additive/backward-compatible**
(legacy 7-day tokens keep working). Detail in the `security-hardening-four-pillars` memory + `SECURITY_HARDENING_PLAN.md`.

- **Authentication**: JWT **pinned to HS256** (sign+verify); teacher tokens carry `kind:'teacher'`; **session revocation** via
  new `User.tokenVersion` (`verifyToken` is now ASYNC + checks it; `change-password` bumps it **and returns a fresh token** — the
  frontend `changePassword` stores it; `reset-password` bumps it → all old sessions die); **cookie-token trust removed** (Bearer-only,
  no CSRF); login runs a dummy `bcrypt.compare` on unknown emails (anti-timing); weak-password denylist; `JWT_SECRET` floor 16→32.
- **Rate Limiting**: **`clientIp` now uses `req.ip`** (was the spoofable first `X-Forwarded-For` token — fixed all IP-keyed limiters);
  new limits on add-student (email-bomb), schedule/email-me, teacher→student messages, checkout, publish, website-create, wizard-draft;
  per-email login counter; **global `/api` 1000/min/IP backstop**.
- **Row-Level Security**: audit found **no cross-tenant IDOR** (all queries `websiteId`-scoped, ownership-gated, cascade complete on
  all 13 tables); `requireStudent` hardened fail-closed. Added cross-tenant regression tests.
- **Server-Side Validation**: `configuration`/`businessConfig` bounded ~2 MB (`boundedRecord()`); settings `breakTimes`/`workingHours`
  get `HH:MM` + weekday-allowlist; search/name/phone/dataUrl/presetId/uuid bounds. Shared `utils/validation.ts`.
...
### Update — video re-cut to v3 (July 12, cinematic/GSAP)
On further feedback (v2 too word-heavy/not creative enough), the video was rebuilt as a **~34s cinematic "chaos → calm" film with only ~5 words**, motion-led. New technique: a **GSAP-driven, frame-stepped renderer** (a license-clean mini-Remotion) — `film-v3.html` runs a `gsap.timeline({paused:true})` choreographing real screenshots + CSS/SVG motion; `render-v3.mjs` seeks it frame-by-frame in Playwright → PNG sequence → ffmpeg + numpy music (`music-v3.py`/`build-v3.py`). (Research: GSAP 100% free since Apr 2025; Remotion source-available so avoided; Motion Canvas weak headless render.) Beats: phone notification chaos → implodes into the Mumotor **M** → **Eli's open-road site builds itself** → a slot is tapped + checkmark (**"Booked."**) → schedule → **daily-report email** slides in → **"You asked. We listened."** → offer chip + mumotor.com. Also (user request): **removed the play-overlay on the decorative hero video** — the demo now opens only via the **"Watch the demo"** button; `hero/CinematicHero.tsx` cache-busts sources with `?v=3`. **v1 (59s) and v2 (40s) archived** at `marketing/video-archive/marketing-v{1,2}.*` (not served) — revert by copying one over `public/media/marketing.*`.

### Update — video re-cut to v2 (July 11)
On feedback the video was re-cut faster/funnier: a **~40s before→after** film. The featured teacher site is now a fictional **"Eli's Driving School" on the `open-road` template** (retro/warm — deliberately NOT the mumotor look, so viewers see it's the *teacher's* site), with a realistic persona/students. Adds the funny **"old way"** open (a stylized generic chat + missed calls — no real WhatsApp brand), a **before/after split**, and the **daily-report email** (faithful recreation of `sendEnhancedDailyReport`, with Eli's students + tap-to-call numbers). Varied kinetic layouts (full-bleed / split / phone-center), not the old text-left/media-right. New faster/playful music. Sources: scratchpad `capture-v2.mjs`/`scenes-v2.html`/`render-v2.mjs`/`music-v2.py`/`build-v2.py`. **The 59s v1 is archived at `marketing/video-archive/marketing-v1.*`** (git-tracked, NOT under `public/`, not served) in case we revert. `public/media/marketing.{mp4,webm}` + poster now hold v2; the hero embed is unchanged.
