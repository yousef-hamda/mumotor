...
**Landing demo video (July 11, re-cut to v2):** `public/media/marketing.{mp4,webm}` + poster — a fast (~40s) funny before/after film built from REAL product screenshots (Playwright → HTML scenes → numpy-synth music → ffmpeg; sources in the session scratchpad `scenes-v2.html`/`music-v2.py`/`build-v2.py`). Story: the messy "old way" (a stylized generic chat + missed calls, no real brand) → a fictional instructor **"Eli's Driving School" on the `open-road` template** (deliberately NOT the mumotor look, so it reads as the *teacher's* site) → students self-booking → a **faithful daily-report email** (matches `sendEnhancedDailyReport`) → the free-month offer + "You asked. We listened." Varied kinetic layouts (full-bleed/split/phone-center), not text-left/media-right. The earlier 59s cut is archived at `marketing/video-archive/marketing-v1.*` (git-tracked, NOT served). `components/VideoLightbox.tsx` + `hero/CinematicHero.tsx` embed it via the **"Watch the demo"** button + hero play-overlay.
