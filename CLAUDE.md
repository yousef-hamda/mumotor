...
**Landing demo video (July 15 — trilingual, language-aware, explains create+get-started):** `public/media/marketing.{en,he,ar}.{mp4,webm}` + posters — a ~42s cinematic film that follows the teacher's journey (pick a design → add details → publish → share your enroll link → students book → daily schedule email → offer), frame-filling, new logo. **The demo follows the site language**: `hero/CinematicHero.tsx` picks `marketing.${i18n.language}.*` (en/he/ar, fallback en, cache-bust `?v=5`). HE/AR are FULLY localized — translated captions (RTL) AND localized product screenshots (HE/AR Eli sites). Built with the GSAP frame-stepper: `film-multi.html?lang=…` (one film + a `T` caption dictionary) → `render-multi.mjs <lang>` → `build-multi.py <lang>`; localized captures via `setup-eli-lang.mjs`+`capture-lang.mjs`. GOTCHA: bake ONE caption DOM element per beat — a shared caption set at build time bleeds onto every scene on seek. Older cuts archived in `marketing/video-archive/`. Details in the `free-trial-and-marketing-video` memory.
