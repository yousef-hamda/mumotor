...
- **Landing demo video** — the homepage's **"Watch the demo"** opens a fast (~40s) before/after product film built from the real UI (`public/media/marketing.mp4`): the messy "old way" (chat/calls) → a real instructor's site (a distinct template + persona) → students self-booking → the daily schedule email — ending on the free-month offer. Replaces the old "live demo" link. (An earlier longer cut is archived at `marketing/video-archive/`.)
