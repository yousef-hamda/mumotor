...
- **Landing demo video** — the homepage's **"Watch the demo"** opens a ~34s **cinematic, motion-led** product film (`public/media/marketing.mp4`) built with a GSAP-driven frame-stepped renderer over the real UI: notification chaos → the Mumotor mark forms → an instructor's site builds itself → a lesson gets booked → the daily schedule email arrives → the free-month offer. Almost no words; motion tells the story. (Earlier cuts archived under `marketing/video-archive/`.)
