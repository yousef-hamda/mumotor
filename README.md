...
- **18 distinct website templates**, each with its own world, palette, typefaces and a signature scroll interaction — the **`mumotor`** brand template (Apple-minimal, with an on-card accent-colour picker) plus **13 signature designs**: `meridian` (topographic survey with a route line that draws itself), `bezel` (a machined instrument whose stat dials sweep), `solari` (a split-flap departures board), `cadence` (kinetic variable-font typography), `circuit` (motorsport telemetry — a car laps a hand-drawn circuit as you scroll), `press` (letterpress type debossed into cotton paper), `reel` (35mm cinema with a scroll-scrubbed filmstrip), `slate` (a chalkboard whose road-diagrams draw themselves), `primary` (Bauhaus geometry), `gallery` (a museum exhibition with a spotlight that follows your scroll), `gilt` (foil-stamped luxury), `sumi` (sumi-e ink wash with a brushed enso), `atelier` (a bespoke tailor's atelier — warm ivory paper, a thread-red accent, a measuring-tape rail and a seam that sews itself as you scroll), `nocturne` (celestial night-drive navigation — deep midnight indigo, gold constellations that light and connect along a drawn course line over a parallax starfield), `deco` (the 1920s golden age of motoring — champagne ivory with emerald-and-gold Art-Deco geometry, sunburst fans and an elevator floor-dial that ticks by section as you scroll), and two kept originals (`grid-ink`, `open-road`). Every template is **full-width desktop** (fills the frame, full-bleed nav/bands) with a generous on-theme nav and a hero that animates on load, a self-contained responsive long-scroll site (hero · stats · packages · about · areas · reviews · gallery · FAQ · booking · contact) rendered from a shared `TemplateData` contract, and defaults to real driving-lesson photography.
...
- **Landing demo video (trilingual, language-aware walkthrough)** — the homepage's **"Watch the demo"** opens a ~68s device-framed product walkthrough with a guiding cursor, casual subtitles, and calm background music: manual-booking pain → pick a design → fill your details → customize → publish → share your enroll code → a student books on their phone → you get pinged → the nightly schedule email → the free-month offer. It **plays in the site's language** — English, Hebrew, or Arabic (RTL), each built from **real localized screenshots** of the app (`public/media/marketing.{en,he,ar}.*`, selected by `i18n.language`). Built with a GSAP-driven frame-stepped renderer over the real UI (`marketing/demo60/`). (Earlier cuts archived under `marketing/video-archive/`.)
