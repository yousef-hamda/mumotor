...
export function StatusBadge({ status }: { status: EnrollmentStatus | string }) {
  return <span className={cn('chip', statusStyles[status] ?? 'bg-zinc-200 text-zinc-600')}>{status}</span>;
}
...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md animate-fade-in rounded-2xl bg-white p-6 shadow-2xl">
